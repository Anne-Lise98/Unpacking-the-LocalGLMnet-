#Computing various LocalGLMnets and analysing the various covariates.

#The code for the LocalGLMnets is inspired by the tutorial written by Daniel Meier and Jürg Schelldorfer.
#this tutorial contains the code corresponding to the article "Insights from Inside Neural Networks". 
...
col_features <- c("AgephNN", "FuelNN", "UseNN", "FleetNN", "SexNN", "BMNN", "Age_carNN", "PowerNN",  "RandNN", "RandUN", "Coverage1", "Coverage2", "Coverage3", "region1",
                  "region2", "region3", "region4", "region5","region6", "region7", "region8", "region9", "region10", "region11") 
...
#Defining the features included in the reduced model. Discussed before, we drop the variable Sex, Fuel, Fleet and Use.
col_features_reduced <- c("AgephNN", "BMNN", "Age_carNN", "PowerNN", "Coverage1", "Coverage2", "Coverage3", "region1",
                          "region2", "region3", "region4", "region5","region6", "region7", "region8", "region9", "region10", "region11")  

col_names_reduced <- c("Ageph", "BM", "Age_car", "Power", "Coverage1", "Coverage2", "Coverage3", "region1",
                       "region2", "region3", "region4", "region5","region6", "region7", "region8", "region9", "region10", "region11") 
...
##### 8. Covariate contributions of the original model ####
#Selecting the submodel that only exists of the unweighted regression attentions
zz <- keras_model(inputs = model_localglm12$input[[1]], outputs = get_layer(model_localglm12, 'attention')$output)
summary(zz)

#Defining the features included in the model. 
...
#Selecting the weight corresponding to the linear layer.
get_weights(model_localglm12)
ww <- as.numeric(get_weights(model_localglm12)[[7]])
...
p2 <- covariatecontributions(6)
p3 <- covariatecontributions(7)
p4 <- covariatecontributions(8)
...
l4 <- covariatecontributions_binary(5)

grid.arrange(l1,l2,l3,l4)

##### 9. Analysis of the two categorical features ####

#Defining a data frame containing the regression attentions for the Coverage for the test data
Beta_Coverage <- cbind(test$Coverage, beta[,9:11]*beta[,31:33])
...
Beta_region <- cbind(test$region, beta[,12:22]*beta[,34:44])
Beta_region$Betaregion <- rowSums(Beta_region[,-1])
Beta_region <- Beta_region[, c(1,13)]
...
ggplot(dat_plt, aes(x = region, y = Betaregion)) + geom_boxplot() +
  ...
  labs(title = paste0("Covariate contribution: ", "region"),
       x = paste0("region", " x"),
       y = "covariate contribution beta(x) * x") + theme(axis.text.x= element_text(size = 6.5))
...
col_names_continuous <- col_names[continuous]
col_names_binary <- col_names[binary]
...
for (jj in 1:length(col_names_continuous)) {
  ...
  grad_beta <- grad_beta[, binary]
  names(grad_beta) <- paste0("Grad", col_names_binary)
...
beta_j <- Attention %>% layer_lambda(function(x) x[, continuous[1]])
model_grad1 <- keras_model(inputs = c(Input), outputs = c(beta_j))
grad <- beta_j %>% layer_lambda(function(x) k_gradients(model_grad1$outputs, model_grad1$inputs))
