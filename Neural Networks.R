#Computing various neural networks to predict the number of claims. 

#The code for the neural networks is inspired by the tutorial written by Daniel Meier and Jürg Schelldorfer.
...
#Fitting the model to the learning data. We consider 300 epochs and batch size = 130 583.
...
#Fitting the model to the learning data. We consider 300 epochs and batch size = 26 116 
#Validation data = 0.2 of the learning data.
fit2 <- model_sh2 %>% fit(
  list(Xlearn, as.matrix(log(learn$Expo))), as.matrix(learn$NClaims),
  epochs = 300,
...
#Fitting the model to the learning data. We consider 300 epochs and batch size = 13 058 
#Validation data = 0.2 of the learning data.
fit3 <- model_sh3 %>% fit(
  list(Xlearn, as.matrix(log(learn$Expo))), as.matrix(learn$NClaims),
  epochs = 300,
...
#Fitting the model to the learning data. We consider 300 epochs and batch size = 1305.
#Validation data = 0.2 of the learning data.
fit5 <- model_sh5 %>% fit(
  list(Xlearn, as.matrix(log(learn$Expo))), as.matrix(learn$NClaims),
  epochs = 300,
...
#Fitting the model to the learning data. We consider 300 epochs and batch size = 261. 
#Validation data = 0.2 of the learning data.
fit6 <- model_sh6 %>% fit(
  list(Xlearn, as.matrix(log(learn$Expo))), as.matrix(learn$NClaims),
  epochs = 300,
...
p5 <- plot_freq(test, "Sex", "frequency by sex", "Neural network", "fit_dp1")
...
p9 <- plot_freq(test, "region", "frequency by region", "Neural network", "fit_dp1")
