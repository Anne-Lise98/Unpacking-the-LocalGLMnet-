#This file considers the data pre-processing for the data set consisting of Belgian car insurance policies. 
#We note that this data pre-processing is inspired by the pre-processing done by J. Schelldorfer and M. V. Wüthrich,
#in the code corresponding to their paper "LocalGLMnet: a deep learning architecture for actuaries". 
#This code can be found here: https://github.com/JSchelldorfer/ActuarialDataScience/tree/master/10%20-%20LocalGLMnet
...
#Making sure that number of claims is an integer 
...
#Adding a new variable indicating the region based on the postcode.
...
#Adding a new variable binning the exposure to simplify the plots. 
...
#Plotting histogram in function of the age of the policyholders for the number of policies.
#We observe that there are not a lot of policies for young and old drivers. 
...
#Histogram for the total exposure. This histogram looks like the histogram for the number of policies, which is intuitive.
...
#Empirical frequency per age policyholder. 
#We observe a very high frequency for drivers of age 18. These young drivers have an empirical frequency of 1.08. 
...
#Looking at the data it is clear that these drivers have a rather short exposure whilst regularly filing claims. 
#As a result, a high frequency is obtained.
#Since these drivers are young, it is intuitive that they would indeed regularly have an accident. 
#We note that the dataset only contains 16 policies for drivers of age 18, which is a very small number.
...
#For drivers older than 92 the empirical frequency is zero. Again, not many policies were obtained 
#for these drivers but having zero accidents seems strange since we expect older drivers to indeed be less safe 
#than slightly younger drivers.  
#Therefore we will cap the age at age 88 -> making sure that this does not distort the model. 

#The empirical severity given that there is a claim 
#for the policyholders per age category. 
#Clearly, since for policyholder with age >92 there are no claims the value for severity will be NaN.
...
#It is visible that the empirical frequency does not differ a lot between the different coverage types. 
...
#Empirical severity per coverage type. Clearly, we expect the severity to be slightly less for drivers with PO coverage.
...
#It is visible that the empirical frequency does not differ a lot between the different usage types. 
#Therefore, we expect that this variable does not have a big impact on the frequency of an accident.
...
################### 5. Fleet ##################
#Most drivers do not belong to a fleet, about 96.8% of the policyholders. 
summary(data$Fleet)
...
#It is visible that the empirical frequency is a bit higher for drivers who do not belong to a fleet. 
#The difference between the empirical frequency of drivers that are part of a fleet 
...
#for drivers who do not belong to a fleet than for the ones who do.
...
#is slightly higher than for males.
...
#Further, we observe very few policies with a bonus-malus level higher than 18.
...
#Analysing these drivers, it is clear that there is one driver with a claim amount of 407 477 resulting into an extremely high severity 
#for this group of drivers. To not distort the data analysis by 
#this one extreme observation, we capped the data for BM at 18.
...
#We observe very few policies for cars that are older than 25 years, only 281. 
#This attributes to approx. 0.17% of the total number of policies.
...
#For the cars of age 37 we observed one claim, resulting in a frequency. 
#Since we do not have a lot of observations for cars older than 25 
#we will cap the data at this point to avoid distortion of the analysis 
...
#Empirical severity per car age. Clearly, looking at the raw data 
#we observe an enormous peak for car age = 37. To not distort the data we cap the data at car age = 25. 
...
#to have a more agessive driving style resulting into more accidents.
#For cars with power = 152 we observe an enormous peak resulting from a driver filing 3 accidents with a very short exposure. 
#To avoid distortion of the analysis by this one observation, we will cap the data at Power = 125.

#Furthermore, we note that there is only one policyholder with power = 10 resulting in a rather high frequency. 
#To avoid distortion, we cap the data at power = 13. 
...
#For the capped data we observe an increasing trend in frequency in function of the power of the car, which is intuitive. 

#Empirical severity per power. We observe a number of peaks. For observations 
#with power > 125 there is only one peak. 
...
#there is one driver with a claim amount of 200 754, resulting in a very large peak. 
#Analysing the policyholder with power = 63, there is one claim of claim amount 1 000 000 attributing to this peak. 
...
#uniformly distributed across the Belgian regions with the exception of Hainaut. 
#For this province there seem to be more policies than for the other provinces.
number_policies_Region<- ggplot(data) + geom_bar(aes(x = region), color = KULbg, fill = "blue", alpha = .5, stat = "count") + 
  labs(x = 'Region', y = 'Number of policies') + ggtitle('Policies per region') + theme(axis.text.x= element_text(size = 6.5))
...
#the empirical frequency between the various provinces. The amounts seem to be 
...
freq_Region <- ggplot(data %>% group_by(region) %>% summarize(Freq = sum(NClaims) / sum(Expo)),aes(x = region, y = Freq))+ geom_bar(stat = "identity",color = KULbg, fill = "blue", alpha = .5)+ 
  labs(x = 'Region', y = 'Empirical frequency') + ggtitle('The empirical frequency per region')+ theme(axis.text.x= element_text(size = 6.5))
...
sev_Region <- ggplot(data %>% group_by(region) %>% summarise(sev = sum(Claim)/sum(NClaims)), aes(x = region, y = sev)) + geom_bar(stat = "identity",color = KULbg, fill = "blue", alpha = .5)+ 
  labs(x = 'Region', y = 'Empirical severity') + ggtitle('The empirical severity per region')+ theme(axis.text.x= element_text(size = 6.5))

sev_Region

################# 11. Spatial view ######################
#This function can be used to read in the shape file of belgium
#This function is replicated from the function created by K. Antonio, R. Henckaerts and R. Verbelen for the R workshop "Pricing analytics in R".
#The original code can be found here: https://katrienantonio.github.io/PE-pricing-analytics/background/2019_04_APC_Pricing_analytics_in_R.html
...
#Mapping Belgium with the total exposure bins
...
belgium_shape@data$sev_class <- cut(belgium_shape@data$sev, breaks = quantile(belgium_shape@data$sev, 
                                                                              c(0,0.2,0.8,1),na.rm = TRUE),right = FALSE, include.lowest = TRUE, labels = c('low','average','high'))
#Mapping the Belgium with the empirical severities
belgium_shape_f <- fortify(belgium_shape)
belgium_shape_f <- left_join(belgium_shape_f,belgium_shape@data)
plot.eda.map <- ggplot(belgium_shape_f, aes(long,lat, group = group)) +
...
#Number of policies per number of claims
...
#Total exposure per number of claims
...
#The number of claims ranges from 0 up to 5. As expected, most policyholders have zero claims.
#Moreover, the exposure and number of policies decreases in function of the number of claims. 
sum(data_cap$NClaims == 0)/nrow(data_cap)

nclaims_table <- data_cap %>% group_by(NClaims) %>% summarise(totalexposure = sum(Expo), n = n())

#Number of policies per exposure
...
#As expected most policies have an exposure of 1. Looking at the histogram for the binned exposures, the number of policies seems to be 
...
#The average claim amounts have the same range. We observe that this very big claim is generated by 1 claim made by a policyholder.
...
#Standardizing the continuous and binary variables for the learning data set
learn <- learn %>% mutate(
...
#Standardizing the continuous and binary variables for the testing data set
test <- test %>% mutate(
...
#variables with respectively 3 and 11 levels.
#To model coverage we add 3 columns to the dataset. For region we add 11. 
